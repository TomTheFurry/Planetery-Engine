...
void StringBox::render() {
	if (_change) {
		...
		_textureSize = uvec2(glm::ceil(
		  vec2(gl::target->viewport.z, gl::target->viewport.w) * size));
		if (_textureSize.x <= 0 || _textureSize.y <= 0) {
			_change = false;
			return;
		}

		if (_targetText == nullptr) { _targetText = new gl::RenderTarget(); }
		if (_fbo == nullptr) _fbo = new gl::FrameBuffer();
		_targetText->bind(_fbo);
		_targetText->viewport = vec4(vec2(0.f), _textureSize);
		_targetText->pixelPerInch = _ppi;
		if (_tex != nullptr) _tex->release();
		_tex = new gl::Texture2D();
		_tex->setFormat(GL_R8, _textureSize.x, _textureSize.y, 1);
		_tex->setTextureSamplingFilter(gl::Texture::SamplingFilter::linear,
		...
		if (_isLineCentre) {
			font::drawStringLineCentre(utf::beginOfUTF8(_str),
			  utf::endOfUTF8(_str), _pointSize, vec2(-1, 1), vec2(2, 2));
		} else {
			font::drawString(utf::beginOfUTF8(_str), utf::endOfUTF8(_str),
			  _pointSize, vec2(-1, 1), vec2(2, 2));
		}
		_change = false;  // disable this to make it always refeashing
	}
	// use render texture here
	float wOverH =
	  float(gl::target->viewport.z) / float(gl::target->viewport.w);
	vec2 trueThick = vec2(borderThickness.x / wOverH, borderThickness.y);
	if (borderThickness == vec2(0)) {
		gl::drawRectangles(
		  {gl::GLRect{.pos = pos, .size = size * 2.f}}, backgroundColor)
		  ->release();
	} else if (backgroundColor == vec4(0)) {
		gl::drawRectanglesBorder({gl::GLRect{.pos = pos - trueThick,
								   .size = (size + trueThick) * 2.f}},
		  trueThick, borderColor)
		  ->release();
	} else {
		gl::drawRectanglesFilledBorder({gl::GLRect{.pos = pos - trueThick,
										 .size = (size + trueThick) * 2.f}},
		  backgroundColor, trueThick, borderColor)
		  ->release();
	}
	gl::drawTexR8Rectangle(_tex, gl::GLRect{.pos = pos, .size = size * 2.f},
	  vec4(0.f, 0.2f, 0.f, 1.f))
	  ->release();
}

std::string StringBox::str() const { return _ss.str(); }
...
vec2 StringBox::getSize() const { return size; }
void StringBox::setTextSize(float p) {
	_change = true;
	_pointSize = p;
}

void StringBox::notifyPPIChanged() { _change = true; }
