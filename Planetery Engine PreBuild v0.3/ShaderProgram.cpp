...
void ShaderProgram::setFloatArray(str valueNameP, float* p, uint length) const
{
	glUniform1fv(glGetUniformLocation(id, valueNameP.c_str()), length, p);
}

void ShaderProgram::setIvec2(str valueNameP, ivec2 value) const
{
	glUniform2i(glGetUniformLocation(id, valueNameP.c_str()), value.x, value.y);
}

void ShaderProgram::setVec2(str valueNameP, vec2 value) const {
...
const float ShaderProgram::boxVertix[] = {
	-1.0f,-1.0f,
	-1.0f,1.0f,
	1.0f,-1.0f,
	1.0f,1.0f
};
const GLenum ShaderProgram::vaoMode = GL_TRIANGLE_STRIP;

void ShaderProgram::initClass()
{
	/*
	boxVId = 0;
	glGenVertexArrays(1, &boxVaoId);
	glBindVertexArray(boxVaoId);
	uint buffer;
	glGenBuffers(1, &buffer);
	glBindBuffer(GL_ARRAY_BUFFER, buffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(boxVertix), boxVertix, GL_STATIC_DRAW);
...
void ShaderProgram::loadFile()
{
	...
	id = tempId;
	shaderInit = true;
	logger("LOG::SHADER::PROGRAM::LOADING_SUCCESSFUL:", id, "\n");
