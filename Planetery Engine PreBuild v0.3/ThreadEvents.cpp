...
inline void _setFullScreenMode(FullScreenMode v) {
	...
	switch (v) {
	case FullScreenMode::windowed:
		glfwSetWindowMonitor(_window, NULL, _windowedPos.x, _windowedPos.y, _windowedSize.x, _windowedSize.y, 0);
		break;
	case FullScreenMode::fullscreen:
		glfwSetWindowMonitor(_window, _targetMonitor, 0, 0, vMode->width, vMode->height, vMode->refreshRate);
		break;
	case FullScreenMode::windowedFullscreen:
		glfwSetWindowAttrib(_window, GLFW_DECORATED, GLFW_FALSE);
		glfwSetWindowMonitor(_window, NULL, 0, 0, vMode->width, vMode->height, vMode->refreshRate);
		break;
	default:
		assert(false);
	}
...
static void _main() {
	try {
		...
		glfwWindowHint(GLFW_AUTO_ICONIFY, GLFW_TRUE); //On fullscreen mode, iconify when lost focus?
		glfwWindowHint(GLFW_FLOATING, GLFW_FALSE); //Window always on top?
		glfwWindowHint(GLFW_MAXIMIZED, GLFW_FALSE); //Window maximized on start?
		glfwWindowHint(GLFW_CENTER_CURSOR, GLFW_TRUE); //On fullscreen mode, centre mouse on start?
		glfwWindowHint(GLFW_FOCUS_ON_SHOW, GLFW_FALSE); //When called show window, focus input?
		glfwWindowHint(GLFW_SCALE_TO_MONITOR, GLFW_TRUE); //Dot Per Inch messurement use monitor value instead of OS value?

		//openGL settings
		glfwWindowHint(GLFW_CLIENT_API, GLFW_OPENGL_API); //Use what rendering api?
		glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE); //use what type of openGL? full compitibility?
		glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GLFW_TRUE); //remove all deprecated functions in openGL?
		...
		glfwWindowHint(GLFW_CONTEXT_RELEASE_BEHAVIOR, GLFW_RELEASE_BEHAVIOR_NONE); //Should openGL flush on buffer release?
		glfwWindowHint(GLFW_OPENGL_DEBUG_CONTEXT, IS_DEBUG_MODE ? GLFW_TRUE : GLFW_FALSE); //is openGL in debug mode?
		glfwWindowHint(GLFW_CONTEXT_ROBUSTNESS, IS_DEBUG_MODE ? GLFW_LOSE_CONTEXT_ON_RESET : GLFW_NO_ROBUSTNESS); //should openGL care about out of bound issue? what should it do?
		glfwWindowHint(GLFW_CONTEXT_NO_ERROR, IS_DEBUG_MODE ? GLFW_FALSE : GLFW_TRUE); //should openGL not check error?

		//render settings
		glfwWindowHint(GLFW_SAMPLES, 4); //gl sub sampling?
		glfwWindowHint(GLFW_SRGB_CAPABLE, GLFW_TRUE); //OpenGL can use sRGB color instead of RGB?
		glfwWindowHint(GLFW_DOUBLEBUFFER, GLFW_TRUE); //Use double buffering?
		glfwWindowHint(GLFW_REFRESH_RATE, GLFW_DONT_CARE); //On fullscreen mode, what refresh rate? (DONT_CARE = max)
		...
		//glfwSetInputMode(_window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); //Main thread only
		...
		while (!glfwWindowShouldClose(_window) || _state.load(std::memory_order_relaxed)==State::requestStop) {
			...
			for (auto& p : _threads) {
				...
				if (d.CallbacksEmpty.load(std::memory_order_relaxed)==false) {
					...
					_activeCallbacks.insert(_activeCallbacks.end(), vec.begin(), vec.end());
					vec.clear();
				}
			}
			...
			glfwWaitEvents(); //Will BLOCK, call glfwPostEmptyEvent() to wake thread
...
bool ThreadEvents::isWindowMoved() {
	return _loadedFlag & _flag::windowMoved;
}
...
bool ThreadEvents::isMouseMoved() {
	return _loadedFlag & _flag::mouseMoved;
}
...
void events::ThreadEvents::setWindowed() {
	_fullScreenModeTarget.store(FullScreenMode::windowed, std::memory_order_release);
	_fullScreenModeChangeRequest.store(true, std::memory_order_relaxed);
}
void events::ThreadEvents::setFullScreen() {
	_fullScreenModeTarget.store(FullScreenMode::fullscreen, std::memory_order_release);
	_fullScreenModeChangeRequest.store(true, std::memory_order_relaxed);
}
void events::ThreadEvents::setWindowedFullscreen() {
	_fullScreenModeTarget.store(FullScreenMode::windowedFullscreen, std::memory_order_release);
	_fullScreenModeChangeRequest.store(true, std::memory_order_relaxed);
}
...
bool events::ThreadEvents::pollEvents() {
	...
	for (auto& e : *events) {
		e.func(e.keyCode, e.keyAction, e.keyModFlags);
	}
	return true;
}
