...
static void _main() {
    try {
        ...
        glfwWindowHint(
          GLFW_REFRESH_RATE, GLFW_DONT_CARE);  // On fullscreen mode, what
        ...
        if (render::ThreadRender::getState() != render::State::init) {
            ...
            // set and threadEvents got error. However std::thread will also halt
            // all child thread if not poperly closed...
        }
    }
    glfwTerminate();
...
void ThreadEvents::setFullScreenInline() {
    if (_targetMonitor != NULL
        && _fullScreenMode != FullScreenMode::fullscreen) {  // use NULL here as
                                                             // glfw returns null
        _unsetFullScreenMode(_fullScreenMode);
