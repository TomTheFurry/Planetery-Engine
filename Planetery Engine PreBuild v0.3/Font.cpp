...
public:
	...
	uint _unitPerEM = 0;
	gl::Ptr<gl::Texture2D> texture = nullptr;
	gl::Ptr<gl::ShaderStorageBuffer> ssbo = nullptr;
...
static gl::Ptr<gl::ShaderProgram> _textShader{};
static gl::Ptr<gl::VertexAttributeArray> _vao{};
static gl::Ptr<gl::VertexBuffer> _vbo{};
	...
	for (auto& fontPair : _requireRender) {
		...
		for (auto& gPair : glyphs) {
			...
			if (bin == nullptr) {
				...
				gl::Ptr<gl::Texture2D> newTexture = new gl::Texture2D();
