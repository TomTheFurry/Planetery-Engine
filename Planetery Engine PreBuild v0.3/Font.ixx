...
export namespace font {
	...
	typedef std::function<void(Reader& data, Output& output)>
	  FullStringFunction;
	...
	extern bool addFont(const std::string& fontSetName,
	  const std::string& fileLocation,
	  const std::vector<std::string>& style = {});
	extern bool linkFallbackFont(
	  const std::string& fontSet, const std::string& fallbackFontSet);
	extern bool useFont(
	  const std::string& fontSet = {});	 // leave empty as default
	extern void useStyle(const std::vector<std::string>& style = {});
	extern bool toggleStyle(
	  const std::string& style, bool setTo);  // return previous value
	extern void closeFontSet(const std::string& fontSet);
	extern void closeFont(
	  const std::string& fontSet, const std::vector<std::string>& style = {});
	...
	extern GlyphData getGlyphData(
	  FontFace* fontFace, GlyphId gId, float pointSize);
	extern void renderRequiredGlyph();
	extern void _renderBatch(
	  FontFace* f, std::vector<RenderData> d, float pointSize);
	extern std::pair<FontFace*, GlyphId> getGlyphFromChar(
	  char32_t c, char32_t replacementChar);

	template<CharTypeIter Iter> static void drawChar(
	  Iter it, Iter itEnd, float pointSize, FullStringFunction func) {
		...
		// Finally render char
		if (output.size() > stringReader.indexLookup.size())
			throw "Invalid output from FullStringFunction() call in "
				  "font::drawChar()!";
		for (uint i = 0; i < output.size(); i++) {
			_renderBatch(
			  stringReader.indexLookup.at(i).first, output.at(i), pointSize);
		}
	}

	extern void _drawStringFunction(
	  Reader& r, Output& output, vec2 topLeftPos, vec2 maxSize);
	template<CharTypeIter Iter> static void drawString(
	  Iter it, Iter itEnd, float pointSize, vec2 topLeftPos, vec2 maxSize) {
	...
	extern void _drawStringCentreFunction(
	  Reader& r, Output& output, vec2 topLeftPos, vec2 maxSize);
	template<CharTypeIter Iter> static void drawStringLineCentre(
	  Iter it, Iter itEnd, float pointSize, vec2 topLeftPos, vec2 maxSize) {
		vec2 drawHead = vec2(-1, 1);
		float maxLineHeight = -1;
